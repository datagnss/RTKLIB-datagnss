...
extern int outnmea_gst(uint8_t *buff, const sol_t *sol, const ssat_t *ssat)
{
    gtime_t time;
    double ep[6],pos[3],sum_rms = 0, range_rms = 0;
    int solq,nf;
    double Q[9]={0},P[9]={0};
    char *p=(char *)buff,*q,sum;
	int sat;
    ...
    for (sat = 1; sat <= MAXSAT; sat++) {
        ...
        for (nf = 0; nf < NFREQ; nf++) {
            sum_rms += SQR(ssat[sat - 1].resc[nf]);
            count_rms++;
        }


        for (nf = 0; nf < NFREQ; nf++) {
            sum_rms += SQR(ssat[sat - 1].resp[nf]);
            count_rms++;
        }
